...
export default function MobileSearch({ onClientSelect }: Props) {
  ...
  // Input value updates instantly — deferred value drives the expensive filtering
  const [query, setQuery] = useState('')
  const deferredQuery = useDeferredValue(query)

  // Full shuffled asset list, computed once on mount
  const allAssets = useMemo<AssetWithClient[]>(() => {
  ...
  // Paginated slice for the default grid
  ...
  const visibleAssets = useMemo(() => allAssets.slice(0, page * PAGE_SIZE), [allAssets, page])

  // Filtered results — driven by deferredQuery so typing stays snappy
  ...
  const searchAssets = useMemo(() =>
    q ? matchedClients.flatMap(c => c.ads.map(ad => ({ ad, client: c }))) : [],
    [q, matchedClients]
  )
  ...
  const handleClientSelect = useCallback((c: Client) => onClientSelect(c), [onClientSelect])

  return (
    <>
      {/* Search bar — outside scroll container, always flush with status bar */}
      <div className="flex-shrink-0 px-4 pt-3 pb-2 border-b" style={{ background: bg, borderColor }}>
        <div className="relative">
          ...
          <input
            type="search"
            placeholder="Search brands, services…"
            value={query}
      ...
      <div
        ...
        style={{ background: bg, paddingBottom: 'calc(68px + env(safe-area-inset-bottom))', transition: 'background 0.3s ease' }}
      >
        {!q ? (
          ...
          <>
            ...
            {visibleAssets.length < allAssets.length && <div ref={sentinelRef} className="h-4" />}
          </>
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-[14px]" style={{ color: subColor }}>No results for "{deferredQuery}"</p>
          </div>
        ) : (
          /* ── Search results: matched brands + all their ads ── */
          <>
            <SectionLabel subColor={subColor}>Brands</SectionLabel>
            <BrandGrid clients={matchedClients} onClientSelect={handleClientSelect} cardBg={cardBg} textColor={textColor} subColor={subColor} gridGap={gridGap} />
            {searchAssets.length > 0 && (
              <>
                <SectionLabel subColor={subColor}>Work</SectionLabel>
                <div className="grid grid-cols-3 gap-[1.5px]" style={{ background: gridGap }}>
                  {searchAssets.map(({ ad, client }, i) => (
                    <AssetThumb key={i} ad={ad} cardBg={cardBg} onClick={() => handleClientSelect(client)} />
                  ))}
                </div>
              </>
        ...
        >
          ...
          >
            {(client.igAvatar || client.logo) ? (
              <img src={client.igAvatar || client.logo} alt={client.name} className="w-full h-full object-contain" loading="lazy" />
            ) : (
