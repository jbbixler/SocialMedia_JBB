...
export async function POST(req: NextRequest) {
  try {
    ...
    const html = `
      <h2>New Comment on Portfolio Ad</h2>
      <p><strong>Client:</strong> ${clientName}</p>
      <p><strong>Ad #:</strong> ${adIndex}</p>
      <blockquote style="border-left:3px solid #0095f6;margin:0;padding:8px 16px;background:#f5f5f7;border-radius:0 8px 8px 0">
        ${comment}
      </blockquote>
      <p style="color:#888;font-size:12px">Sent from jbradbixler portfolio — not visible to other users</p>
    `
